<template>
  <div class="permissions" >
    <el-button type="primary" @click="_collapse()">{{collapse?'展开':'收起'}}</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      _collapse(){
        ...
        if(this.collapse){
          this.$store.dispatch('roleNodeEmpty');
        }else{
          let roleParentId = this.$store.state.roleParentId;
          let _pIds = [];
          for(let i in roleParentId){
            let pId = roleParentId[i];
            if(!_pIds.includes(pId)){
              _pIds.push(pId);
              this.$store.dispatch('roleNodeAdd',pId);
            }
          }
        }
      }
